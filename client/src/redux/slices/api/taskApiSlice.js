    ...
    duplicateTask: builder.mutation({
      ...
      async onQueryStarted({ taskId, ...patch }, { dispatch, queryFulfilled }) {
        try {
          ...
          dispatch(
            postApiSlice.util.invalidateTags([{ type: 'Task' }])
          );
        } catch {
          // If the update fails, the error will be handled in the component
        }
      },
    }),getAllTask: builder.query({
      query: ({ strQuery, isTrashed, search }) => ({
        url: `${TASKS_URL}?stage=${strQuery?.toLowerCase() || ''}&isTrashed=${isTrashed}&search=${search}`,
        method: "GET",
        credentials: "include",
      }),
      providesTags: ['Task'],
